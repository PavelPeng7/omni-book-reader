...
4. Desktop mouse or pen selection stays on the current page, including at the viewport edge. Cross-page passages are saved as separate selections.
...
9. A click or tap outside an active native or pending selection dismisses it and consumes that input; clicking within the selection does not dismiss it. Later ordinary navigation is available.
...
Foliate's latest relocation exposes the exact visible text `Range` for the current page. During an Android selection gesture, the reader compares both native selection endpoints with that range and clamps only out-of-page endpoints to its start or end. This preserves selection direction and the in-page endpoint. Clamping happens before viewport scroll restoration. A selection entirely outside the visible range is left alone because it has no in-page endpoint to preserve. Continuous layout, fixed layout, and selections outside a touch gesture or pending annotation are unaffected.

Event tests cover both selection endpoints crossing the previous-page boundary, a forward boundary crossing, ordinary in-page adjustment, and continuous layout. Native Android handle appearance still needs device validation.

## Fixed selection behavior and inverse colors (2026-09-26)

Selection navigation protection is always active. Normalization drops old `preventPageTurnsWhileSelecting` values, including `false`. The document event coordinator records the native selection and whether a pointer or touch begins outside its rendered text rectangles. A completed outside click or tap clears native and pending selection, consumes that input, and resets the page-turn guard. This avoids interpreting the dismissal input as a page turn; a later tap can navigate. Selection-handle drags remain owned by selection. When the optional `invertSelectionColors` setting is enabled, publication CSS uses the resolved reading theme's foreground for selected background and page background for selected text. The ordinary translucent selection style remains the default.
...
| Native selection, outside click or tap | Selection clears without navigation; the next ordinary input may navigate |
